...
export class Delaunay extends CanvasEffect<DelaunayConfig> {
	...
	constructor(item: HTMLCanvasElement, config: DelaunayConfig) {
		...
		this.light = [-1000, -1000, this.apex]
	...
	private setLight(x: number, y: number) {
		this.light = [x, y, this.apex]
	}

	private getApexHeight(): number {
		return (this.canvas!.height + this.canvas!.width) / 4
	}
	...
	private onMouseMove(e: MouseEvent): void {
		if (this.config.mouse) {
			...
			this.setLight((e.clientX - canvasRect.left), (e.clientY - canvasRect.top))
		}
	}
}
